...
export async function buildAndDeploy({ inputDir, outputDir, name, version, stage, role, environment, awsConfig }) {
  await buildClient({
  ...
  const apiURL = await deploy({
    name, version, stage, entryFile: serverIndexFile, role, environment, awsConfig
  });
