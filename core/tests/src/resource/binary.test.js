...
describe('BinaryResource', () => {
  test('creation', async () => {
    ...
    expect(Buffer.isBuffer((await BinaryResource.$create({'@value': Buffer.alloc(0)})).$value)).toBe(true);
    expect((await BinaryResource.$create({'@value': Buffer.alloc(0)})).$value.length).toBe(0);
    expect((await BinaryResource.$create({'@value': Buffer.from([1, 2, 3])})).$value.toJSON()).toEqual({type: 'Buffer', data: [1, 2, 3]});
    expect((await BinaryResource.$create({'@value': 'data:;base64,'})).$value.length).toBe(0);
    expect((await BinaryResource.$create({'@value': 'data:;base64,AQID'})).$value.toJSON()).toEqual({type: 'Buffer', data: [1, 2, 3]});
  ...
  test('serialization', async () => {
    ...
    expect((await BinaryResource.$create(Buffer.from([1, 2, 3]))).$serialize()).toBe('data:;base64,AQID');
  });
});
