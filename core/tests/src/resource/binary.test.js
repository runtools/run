...
describe('BinaryResource', () => {
  test('creation', async () => {
    ...
    expect(Buffer.isBuffer((await Resource.$create({'@value': Buffer.alloc(0)})).$value)).toBe(true);
    ...
    expect((await Resource.$create({'@type': 'binary', '@value': 'data:;base64,'})).$value.length).toBe(0);
    expect((await Resource.$create({'@type': 'binary', '@value': 'data:;base64,AQID'})).$value.toJSON()).toEqual({
      type: 'Buffer',
      data: [1, 2, 3]
    });
    await expect(Resource.$create({'@type': 'binary', '@value': 'hello'})).rejects.toBeInstanceOf(Error);
  });
